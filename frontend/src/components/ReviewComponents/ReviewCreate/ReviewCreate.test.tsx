...
describe('<ReviewCreate />', () => {
    ...
    it("should render button and inputs with original values", async () => {
        ...
        fireEvent.change(titleInput, { target: { value: "ARTICLE_TEST_TITLE" } });

        const contentInput = await screen.findByLabelText("입양 후기를 알려주세요! 자세한 후기는 입양에 도움이 됩니다:)");
        fireEvent.change(contentInput, { target: { value: "ARTICLE_TEST_CONTENT" } });
